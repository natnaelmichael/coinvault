    ...
    async def buy_token(
        ...
        try:
            ...
            if result.success and result.confirmed:
                logger.info(f"✓ {wallet.label}: Buy confirmed! Sig: {result.signature[:16]}...")
                ...
                return BuyResult(wallet=wallet, success=False, signature=result.signature, error=result.error)
